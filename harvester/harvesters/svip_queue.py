...
#CHROM\tPOS\tID\tREF\tALT\tQUAL\tFILTER\tINFO\n"""
    ...
    def as_vcf(records, as_str=False):
        ...
        fp.writelines(SubmittedVariantTransformer.as_vcf_row(rec)
                      for rec in records)
    ...
    def as_vcf_row(rec):
        ...
        original_alt = ",".join(
            [x.strip() for x in rec['alt'].strip("[]").replace("None", ".").split(",")])
...
def _parse_vep_output(data):
    ...
    header = next((x.strip("#")
                  for x in lines if re.match(r'^#[^#].*$', x)), None)
    ...
    for rec in records:
        ...
        if 'Extra' in cells:
            try:
                cells['Extra'] = dict(x.split("=")
                                      for x in cells['Extra'].split(";"))
...
def harvest(genes):
    ...
    try:
        ...
        pg_silo = next(
            silo for silo in harvester.silos if isinstance(silo, PostgresSilo))
    except StopIteration:
        # if it doesn't exist, we can't proceed
        raise Exception(
            "svip_queue harvester can't run without PostgresSilo being in the list of silos")
    ...
    with pg_silo._connect() as conn:
        ...
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as curs:
            curs.execute(
                "select * from svip_submittedvariant where status != 'completed'")
            entries = curs.fetchall()

            logging.info("Processing %d non-complete records: %s" %
                         (len(entries), [x['id'] for x in entries]))
            ...
            process = subprocess.Popen(["/app/scripts/run_curator_vep.sh"],
                                       stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            result, stderr_result = process.communicate(input=vcf_data)

            if process.returncode != 0:
                raise Exception("VEP failed w/code %d; stderr: %s" %
                                (process.returncode, stderr_result))

            # build set of submission IDs which should retain only canonical variants
            # we'll use this for filtering below
            canonical_only_ids = set(int(x['id'])
                                     for x in entries if x['canonical_only'])
            ...
            for rec in variants:
                ...
                try:
                    try:
                        ...
                        raise InvalidVEPRecord(
                            'no gene symbol found in VEP record (consequence: %s)' % rec['Consequence'])

                    if gene_set and this_gene not in gene_set:
                        raise InvalidVEPRecord(
                            "skipped because it wasn't in the harvester's gene set")

                    # skip variants who can't be easily named by a protein change
                    if rec['Protein_position'] == '-' or rec['Amino_acids'] == '-':
                        raise InvalidVEPRecord(
                            'protein position and/or amino acids were unspecified (consequence: %s)' % rec['Consequence'])

                except InvalidVEPRecord as ex:
                    logging.info("Skipping %d, reason: %s" %
                                 (submit_id, ex.message))
                    # pprint(rec)
...
def convert(gene_data):
    ...
    for variant in variants:
        ...
        variant_name = "%s%s%s" % (
            amino_from, variant['Protein_position'], amino_to)
        ...
        feature = {
            ...
            # our VEP db is always GRCh37 (at least for now)
            'referenceName': 'GRCh37',
            # accession(variant['Extra']['HGVSc']), # sometimes we get refseq, sometimes we get ensembl...
            'refseq': None,
            # it's unclear how to get the ensembl transcript identifier from vep, ironically...
            'isoform': None,
        ...
        feat_assoc = {
            ...
            'association': {
                ...
                'description': '%s %s' % (gene_data['gene'], variant_name),
