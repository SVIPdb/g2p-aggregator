...
def acquire_files(data_files, overwrite=False):
    """
    ...
    :param data_files: a dicts with keys corresponding to the files and values describing how to fetch the files
     (described above)
    :param overwrite: whether to redownload the files if they already exist or not
    ...
    for name, meta in data_files.items():
        ...
        try:
            if not os.path.exists(meta['path']) or overwrite:
                ...
                logging.info("Not downloading %s because it already exists" % name)
