"""
New version of the location_normalizer module

Imported from https://github.com/ohsu-comp-bio/g2p-aggregator/blob/v0.12/harvester/location_normalizer.py
"""
import requests
import re
import logging
...
from new_feature_enricher import enrich
from lookups import protein
import time
import hgvs.parser
...
def _get_hgvs_set(clinvar_alleles, attempt=0):
    ...
    for variant_id in variant_ids:
        ...
        url = 'https://eutils.ncbi.nlm.nih.gov/entrez/eutils/efetch.fcgi?db=clinvar&id={}&rettype=vcv'.format(variant_id)
        resp = requests.get(url)
        ...
        resp.raise_for_status()
        tree = ElementTree.fromstring(resp.content)
        hgvs_list = tree.find('VariationArchive').find('InterpretedRecord').find('SimpleAllele').find('HGVSlist')
        hgvs_set.update(
